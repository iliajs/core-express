...
export const createOrUpdateCustomer = async (req, response) => {
  ...
  if (customer.id) {
    pool.query(
      "update customers set updated_at=now() at time zone 'utc', name=$2, price=$3, currency=$4 where id=$1",
      [customer.id, customer.name, customer.price, customer.currency],
      async (error) => {
        if (error) {
    ...
    pool.query(
      "insert into customers(created_at, name, price, currency) values(now() at time zone 'utc', $1, $2, $3) RETURNING id",
      [customer.name, customer.price, customer.currency],
      async (error, result) => {
        if (error) {
...
export const listCustomers = async (request, response) => {
  pool.query(
    "select * from customers where deleted=false order by id desc",
    async (error, res) => {
      if (error) {
...
export const deleteCustomer = async (request, response) => {
  ...
  pool.query(
    "update customers set deleted_at=now() at time zone 'utc', deleted=true where id=$1",
    [id],
    async (error) => {
      if (error) {
