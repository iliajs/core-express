...
export const createAppointmentResult = async (req, response) => {
  ...
  try {
    ...
    await log(
      logObjects.appointmentResult,
      operation === "insert" ? logOperations.insert : logOperations.update,
      id,
      {
    ...
    console.error(
      "error when try to add new appointment result to database",
      e
    );
    response.send({
      success: false,
      error: { text: "error add new appointment result to database" },
    });
  }
};
